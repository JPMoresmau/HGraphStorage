...
spec = do
  describe "Update operations" $ do
    withEmptyDB "creates nodes" $ \db->do
        nnb0 <- testdb db $ nbNodes
        nnb0 `shouldBe` 0
        oid1 <- testdb db $ addNode "type1" [nm "obj0",cnt 1]
        oid1 `shouldBe` 1
        nnb0 <- testdb db $ nbNodes
        nnb0 `shouldBe` 1
        ...
        pNext p2 `shouldBe` 1
        oid2 <- testdb db $ addNode "type1" [nm "obj2",cnt 1]
        o2 <- atomically $ readNode db oid2
        o2 `shouldBe` Node 1 def def 4
    withEmptyDB "creates edges" $ \db->do
        nnb0 <- testdb db $ nbNodes
        nnb0 `shouldBe` 0
        enb0 <- testdb db $ nbEdges
        enb0 `shouldBe` 0
        oid1 <- testdb db $ addNode "type1" [nm "obj1",cnt 1]
        oid2 <- testdb db $ addNode "type1" [nm "obj2",cnt 1]
        eid1 <- testdb db $ addEdge oid1 "ref1" [w 1] oid2
        nnb1 <- testdb db $ nbNodes
        nnb1 `shouldBe` 2
        enb1 <- testdb db $ nbEdges
        enb1 `shouldBe` 1
        eid1  `shouldBe` 1
        ...
        eid2 <- testdb db $ addEdge oid1 "ref2" [w 2] oid2
        eid2  `shouldBe` 2
        e1' <- atomically $ readEdge db eid1
        e1' `shouldBe` Edge oid1 1 oid2 1 1 def def 5
    ...
    withEmptyDB "deletes nodes" $ \db -> do
        oid1 <- testdb db $ addNode "type1" [nm "obj1",cnt 1]
        oid2 <- testdb db $ addNode "type1" [nm "obj2",cnt 1]
        oid3 <- testdb db $ addNode "type1" [nm "obj3",cnt 1]
        eid1 <- testdb db $ addEdge oid1 "ref1" [w 1] oid2
        eid2 <- testdb db $ addEdge oid1 "ref1" [w 1] oid3
        eid3 <- testdb db $ addEdge oid2 "ref1" [w 1] oid3
        nnb0 <- testdb db $ nbNodes
        nnb0 `shouldBe` 3
        enb0 <- testdb db $ nbEdges
        enb0 `shouldBe` 3
        testdb db $ removeNode oid1
        o1 <- atomically $ readNode db oid1
        o1 `shouldBe` def
        o2 <- atomically $ readNode db oid2
    ...
    withEmptyDB "deletes edges" $ \db -> do
        ...
        e3' <- atomically $ readEdge db eid3
        e3' `shouldBe` Edge oid2 1 oid3 1 1 def eid2 9
        testdb db $ removeEdge eid2
        nnb2 <- testdb db $ nbNodes
        nnb2 `shouldBe` 3
        enb2 <- testdb db $ nbEdges
        enb2 `shouldBe` 1
        o1' <- atomically $ readNode db oid1
        o1' `shouldBe` Node 1 def def 2
        e3'' <- atomically $ readEdge db eid3
    ...
    withEmptyDB "updates properties" $ \db -> do
        oid1 <- testdb db $ addNode "type1" [nm "obj0",cnt 1]
        oid2 <- testdb db $ addNode "type1" [nm "obj2",cnt 2]
        eid1 <- testdb db $ addEdge oid1 "ref1" [w 1] oid2
        p1 <- testdb db $ nodeProperties oid1 return
        p1 `shouldBe` [nm "obj0",cnt 1]
        p2 <- testdb db $ nodeProperties oid1 (\ps->return ((s 3) : ps))
        p2 `shouldBe` [s 3,nm "obj0",cnt 1]
        p3 <- testdb db $ nodeProperties oid1 return
        p3 `shouldBe` p2
        p4 <- testdb db $ edgeProperties eid1 return
        p4 `shouldBe` [w 1]
        p5 <- testdb db $ edgeProperties eid1 (\ps->return $ (s 2) : ps)
        p5 `shouldBe` [s 2,w 1]
        p6 <- testdb db $ edgeProperties eid1 return
        p6 `shouldBe` p5
  describe "Node Traversals" $ do
    withEmptyDB "finds node by id" $ \db->do
        oid1 <- testdb db $ addNode "type1" [nm "obj0",cnt 1]
        r <- testdb db $ traverseGraph (Ns <> NID  [1])
    ...
    withEmptyDB "finds node by id and value" $ \db-> do
        oid1 <- testdb db $ addNode "type1" [nm "obj1",cnt 1]
        oid2 <- testdb db $ addNode "type1" [nm "obj2",cnt 1]
        r <- testdb db $ traverseGraph (Ns <> NID  [1,2] <> Has (nm "obj1"))
        r `shouldBe` Nodes [oid1]
    withEmptyDB "finds node by value" $ \db->do
        oid1 <- testdb db $ addNode "type1" [nm "obj1",cnt 1]
        oid2 <- testdb db $ addNode "type1" [nm "obj2",cnt 1]
        r <- testdb db $ traverseGraph (Ns <> Has (nm "obj1"))
        r `shouldBe` Nodes [oid1]
    withEmptyDB "doesn't find node by id and unknown value" $ \db->do
        oid1 <- testdb db $ addNode "type1" [nm "obj1",cnt 1]
        oid2 <- testdb db $ addNode "type1" [nm "obj2",cnt 1]
        r <- testdb db $ traverseGraph (Ns <> NID  [1,2] <> Has (nm "obj3"))
        r `shouldBe` Empty
    withEmptyDB "doesn't find node by unknown value" $ \db->do
        oid1 <- testdb db $ addNode "type1" [nm "obj1",cnt 1]
        oid2 <- testdb db $ addNode "type1" [nm "obj2",cnt 1]
        r <- testdb db $ traverseGraph (Ns <> Has (nm "obj3"))
        r `shouldBe` Empty
  describe "Edge Traversals" $ do
    withEmptyDB "finds edge by id" $ \db->do
        ...
        oid2 <- testdb db $ addNode "type1" [nm "obj2",cnt 1]
        eid1 <- testdb db $ addEdge oid1 "ref1" [w 1] oid2
        r <- testdb db $ traverseGraph (Es <> EID  [1])
    ...
    withEmptyDB "finds edge by id and value" $ \db-> do
        ...
        eid1 <- testdb db $ addEdge oid1 "ref1" [w 1] oid2
        eid2 <- testdb db $ addEdge oid1 "ref2" [w 2] oid2
        r <- testdb db $ traverseGraph (Es <> EID  [1,2] <> Has (w 1))
    ...
    withEmptyDB "finds edge by value" $ \db->do
        ...
        eid1 <- testdb db $ addEdge oid1 "ref1" [w 1] oid2
        eid2 <- testdb db $ addEdge oid1 "ref2" [w 2] oid2
        r <- testdb db $ traverseGraph (Es <> Has (w 1))
        r `shouldBe` Edges [oid1]
    withEmptyDB "doesn't find edge by id and unknown value" $ \db->do
        ...
        eid1 <- testdb db $ addEdge oid1 "ref1" [w 1] oid2
        eid2 <- testdb db $ addEdge oid1 "ref2" [w 2] oid2
        r <- testdb db $ traverseGraph (Es <> EID  [1,2] <> Has (w 3))
        r `shouldBe` Empty
    withEmptyDB "doesn't find edge by unknown value" $ \db->do
        ...
        eid1 <- testdb db $ addEdge oid1 "ref1" [w 1] oid2
        eid2 <- testdb db $ addEdge oid1 "ref2" [w 2] oid2
        r <- testdb db $ traverseGraph (Es <> Has (w 3))
        r `shouldBe` Empty
  describe "Returning values" $ do
    withEmptyDB "get specific values for nodes" $ \db -> do
        oid1 <- testdb db $ addNode "type1" [nm "obj1",cnt 1]
        oid2 <- testdb db $ addNode "type1" [nm "obj2",cnt 1, w 2]
        r <- testdb db $ traverseGraph (Ns <> NID  [1,2] <> Values ["nm","weight"])
    ...
    withEmptyDB "get specific values for all nodes" $ \db -> do
        oid1 <- testdb db $ addNode "type1" [nm "obj1",cnt 1]
        oid2 <- testdb db $ addNode "type1" [nm "obj2",cnt 1, w 2]
        r <- testdb db $ traverseGraph (Ns <> Values ["nm","weight"])
    ...
    withEmptyDB "get specific values for edges" $ \db -> do
        ...
        oid2 <- testdb db $ addNode "type1" [nm "obj2",cnt 1]
        eid1 <- testdb db $ addEdge oid1 "ref1" [w 1] oid2
        eid2 <- testdb db $ addEdge oid1 "ref2" [w 2,s 0] oid2
        r <- testdb db $ traverseGraph (Es <> EID  [1,2] <> Values ["since","weight"])
    ...
    withEmptyDB "unknown properties for edges" $ \db -> do
        ...
        eid1 <- testdb db $ addEdge oid1 "ref1" [w 1] oid2
        eid2 <- testdb db $ addEdge oid1 "ref2" [w 2,s 0] oid2
        r <- testdb db $ traverseGraph (Es <> EID  [1,2] <> Values ["since2","weight2"])
    ...
    withEmptyDB "get all values for edges" $ \db -> do
        ...
        eid1 <- testdb db $ addEdge oid1 "ref1" [w 1] oid2
        eid2 <- testdb db $ addEdge oid1 "ref2" [w 2,s 0] oid2
        r <- testdb db $ traverseGraph (Es <> EID  [1,2] <> AllValues)
    ...
    withEmptyDB "get specific values for all edges" $ \db -> do
        ...
        eid1 <- testdb db $ addEdge oid1 "ref1" [w 1] oid2
        eid2 <- testdb db $ addEdge oid1 "ref2" [w 2,s 0] oid2
        r <- testdb db $ traverseGraph (Es <> Values ["since","weight"])
    ...
    withEmptyDB "unknown properties for all edges" $ \db -> do
        ...
        eid1 <- testdb db $ addEdge oid1 "ref1" [w 1] oid2
        eid2 <- testdb db $ addEdge oid1 "ref2" [w 2,s 0] oid2
        r <- testdb db $ traverseGraph (Es <> Values ["since2","weight2"])
    ...
    withEmptyDB "get all values for all edges" $ \db -> do
        ...
        eid1 <- testdb db $ addEdge oid1 "ref1" [w 1] oid2
        eid2 <- testdb db $ addEdge oid1 "ref2" [w 2,s 0] oid2
        r <- testdb db $ traverseGraph (Es <> AllValues)
  ...
  describe "Out edges" $ do
     withEmptyDB "specific edge types" $ \db -> do
        ...
        oid3 <- testdb db $ addNode "type1" [nm "obj3",cnt 1]
        eid1 <- testdb db $ addEdge  oid1 "ref1" [w 1] oid2
        eid2 <- testdb db $ addEdge oid1 "ref2" [w 2,s 0] oid3
        r0 <- testdb db $ traverseGraph (Ns <> NID  [1] <> Out ["ref1"])
        r0 `shouldBe` Nodes [oid2]
        r1 <- testdb db $ traverseGraph (Ns <> NID  [1] <> Out ["ref1","ref2"])
        r1 `shouldBe` Nodes [oid2,oid3]
        r2 <- testdb db $ traverseGraph (Ns <> Out ["ref1","ref2"])
        r2 `shouldBe` Nodes [oid2,oid3]
        r3 <- testdb db $ traverseGraph  (Ns <> OutE ["ref1","ref2"])
     ...
     withEmptyDB "all edge types" $ \db -> do
        ...
        eid1 <- testdb db $ addEdge oid1 "ref1" [w 1] oid2
        eid2 <- testdb db $ addEdge oid1 "ref2" [w 2,s 0] oid3
        r0 <- testdb db $ traverseGraph (Ns <> NID  [1] <> Out ["*"])
     ...
     withEmptyDB "unknown edge types" $ \db -> do
        ...
        eid1 <- testdb db $ addEdge oid1 "ref1" [w 1] oid2
        eid2 <- testdb db $ addEdge oid1 "ref2" [w 2,s 0] oid3
        r0 <- testdb db $ traverseGraph (Ns <> NID  [1] <> Out ["ref3","ref4"])
        r0 `shouldBe` Empty
        r1 <- testdb db $ traverseGraph (Ns <> NID  [1] <> Out ["ref1","ref3"])
  ...
  describe "In edges" $ do
     withEmptyDB "specific edge types" $ \db -> do
        ...
        oid2 <- testdb db $ addNode "type1" [nm "obj2",cnt 1]
        oid3 <- testdb db $ addNode "type1" [nm "obj3",cnt 1]
        eid1 <- testdb db $ addEdge oid2 "ref1" [w 1] oid1
        eid2 <- testdb db $ addEdge oid3 "ref2" [w 2,s 0] oid1
        r0 <- testdb db $ traverseGraph (Ns <> NID  [1] <> In ["ref1"])
        r0 `shouldBe` Nodes [oid2]
        r1 <- testdb db $ traverseGraph (Ns <> NID  [1] <> In ["ref1","ref2"])
        r1 `shouldBe` Nodes [oid2,oid3]
        r2 <- testdb db $ traverseGraph (Ns <> In ["ref1","ref2"])
        r2 `shouldBe` Nodes [oid2,oid3]
        r3 <- testdb db $ traverseGraph (Ns <> InE ["ref1","ref2"])
     ...
     withEmptyDB "all edge types" $ \db -> do
        ...
        eid1 <- testdb db $ addEdge oid2 "ref1" [w 1] oid1
        eid2 <- testdb db $ addEdge oid3 "ref2" [w 2,s 0] oid1
        r0 <- testdb db $ traverseGraph (Ns <> NID  [1] <> In ["*"])
        r0 `shouldBe` Nodes [oid2,oid3]
     withEmptyDB "unknown edge types" $ \db -> do
        oid1 <- testdb db $ addNode "type1" [nm "obj1",cnt 1]
        oid2 <- testdb db $ addNode "type1" [nm "obj2",cnt 1]
        ...
        eid1 <- testdb db $ addEdge oid2 "ref1" [w 1] oid1
        eid2 <- testdb db $ addEdge oid3 "ref2" [w 2,s 0] oid1
        r0 <- testdb db $ traverseGraph (Ns <> NID  [1] <> In ["ref3","ref4"])
        r0 `shouldBe` Empty
        r1 <- testdb db $ traverseGraph (Ns <> NID  [1] <> In ["ref1","ref3"])
        r1 `shouldBe` Nodes [oid2]
  describe "Both edges" $ do
     withEmptyDB "specific edge types" $ \db -> do
        ...
        eid1 <- testdb db $ addEdge oid2 "ref1" [w 1] oid1
        eid2 <- testdb db $ addEdge oid1 "ref2" [w 2,s 0] oid3
        r0 <- testdb db $ traverseGraph (Ns <> NID  [1] <> Both ["ref1"])
        r0 `shouldBe` Nodes [oid2]
        r1 <- testdb db $ traverseGraph (Ns <> NID  [1] <> Both ["ref1","ref2"])
        r1 `shouldBe` Nodes [oid2,oid3]
        r2 <- testdb db $ traverseGraph (Ns <> Both ["ref1","ref2"])
        r2 `shouldBe` Nodes [oid1,oid2,oid1,oid3]
        r3 <- testdb db $ traverseGraph (Ns <> BothE ["ref1","ref2"])
...
testdb db f = atomically $ withDatabase db f
